...
function AdminLayout({ setView }) {
  ...
  //Estado global para guardar categorías
  const [categories, setCategories] = useState([]);

  //Función mágica que va a Supabase a buscar las categorías 
  ...
  // Trae los productos Y TAMBIÉN las categorías juntas
  useEffect(() => {
    if (user) {
      fetchAdminProducts();
  ...
  return (
    <div className="flex-1 p-4 flex flex-col bg-white m-4 rounded-3xl shadow-xl max-h-[85vh] overflow-hidden">
      <div className="flex justify-between items-center border-b pb-2 mb-2">
        ...
        <button onClick={() => setUser(null)} className="text-xs font-bold text-red-500 bg-red-50 px-3 py-1.5 rounded-xl active:scale-95 transition-transform flex items-center space-x-1 hover:bg-red-100/70"
        >
          <span>Salir</span>
