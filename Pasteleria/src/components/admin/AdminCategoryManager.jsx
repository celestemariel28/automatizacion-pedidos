// src/components/admin/AdminCategoryManager.jsx
  ...
  return (
    <div className="flex flex-col flex-1 h-full overflow-hidden">
      ...
      <form 
        ...
        className={`mb-4 p-3 rounded-xl border transition-colors ${categoryToEdit ? 'bg-amber-50/70 border-amber-200' : 'bg-gray-50 border-gray-100'} space-y-2`}
      >
        <div className="flex justify-between items-center mb-1">
          <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider flex items-center space-x-1.5">
            {categoryToEdit ? (
              <>
                <FolderEdit className="w-3 h-3 text-amber-600" /> {/* Ícono profesional para editar */}
                <span>Editando Categoría</span>
              </>
            ) : (
              <>
                <Tag className="w-3 h-3 text-[#E91E63]" /> {/* Ícono profesional para nueva */}
          ...
          {categoryToEdit && (
            <button 
              ...
              className="text-[10px] text-amber-700 font-bold underline bg-transparent"
            >
              Cancelar
            </button>
          )}
      ...
      {/* LISTADO DE CATEGORÍAS MODULARIZADO */}
